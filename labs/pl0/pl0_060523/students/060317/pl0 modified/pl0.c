...
void factor(symset fsys)
{
	...
	int i;
	symset set;
	symset set1; // added by xwang 06-03-17
...
// modified by xwang 06-03-17
			expression(set);
			destroyset(set);
			destroyset(set1); // added by xwang 06-03-17
			if (sym == SYM_RPAREN)
			{
				getsym();
			}
	...
	do
	{
		...
		case OPR:
			...
			case OPR_LEQ:
				top--;
				stack[top] = stack[top] <= stack[top + 1];
				 break; // added by xwang 06-03-17, not very needed, but clearly
			} // switch
			break;
		case LOD:
